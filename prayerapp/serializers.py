...
class CardSerializer(serializers.ModelSerializer):
    ...
    copyright_notice = serializers.SlugRelatedField(
        slug_field="copyright_notice", read_only=True, source="bibleversion"
    )
...
def get_scripture_text(card: Card, bible_version: BibleVersion) -> Any:
    ...
    for ref in card.scripture.split(";"):
        if ref.strip():
            if ref_match := re.match(r"^([1-3]? ?[a-zA-Z]+)\.? ?([0-9]+):([0-9]+)$", ref.strip()):
                ...
                verses |= BibleVerse.objects.filter(
                    ...
                    verse__lte=verse_end,
                    version=bible_version,
                )
            elif ref_match := re.match(
                r"^([1-3]? ?[a-zA-Z]+)\.? ?([0-9]+):([0-9]+)-([0-9]+):([0-9]+)$",
                ref.strip(),
            ):
                book = get_book_abbreviation(ref_match.group(1))
